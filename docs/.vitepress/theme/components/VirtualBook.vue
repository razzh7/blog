<template>
  <div class="bookshelf">
    <div class="book" v-for="book of books" :key="book.title">
      <div
        class="book__side spine"
        :style="{ backgroundColor: book.spine || '' }"
      >
        <span class="spine-title">{{ book.title }} </span>
        <span class="spine-author"> {{ book.tag }} </span>
      </div>
      <div class="book__side book__top"></div>
      <div
        class="book__side book__cover"
...
.book:hover {
  z-index: 1;
  transform: rotateX(-25deg) rotateY(-40deg) rotateZ(-15deg)
